...
const router = Router();
const METODO_PAGO = ['Efectivo', 'Transferencia', 'Debito', 'Credito', 'scholarship']
...
router.post('/', (req: Request, res: Response, next)  => {
    ...
    if(!METODO_PAGO.includes(metodo_pago)){
        ...
        res.status(400).json({
            error: "El metodo de pago insertado debe ser: efectivo, debito, credito, transferencia o scholarship"
    });
}
    res.status(201).json({
        version: 'v2',
